            ...
            .then(data => {
                ...
                data.forEach(product => {
                    ...
                    productElement.innerHTML = `
                        ...
                        <button class="button" data-product-id="${product.id}" data-quantity-id="quantity-${product.id}">Buy Now</button>
                    `;
                    // Append the new product div to the container for display
                    productsContainer.appendChild(productElement);
                });

                // Add event listeners to the dynamically created "Buy Now" buttons
                document.querySelectorAll('.button').forEach(button => {
                    button.addEventListener('click', function () {
                        const productId = this.getAttribute('data-product-id');
                        const quantity = document.getElementById(this.getAttribute('data-quantity-id')).value;
